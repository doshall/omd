...
- **桌面托盘**：补全 `TrayIcon` 类型导入，修复 macOS / Windows Release 编译失败（Linux 因 `cfg` 未引用该类型而未暴露问题）

### 变更

- 版本号升至 **0.9.3**；Android `versionCode` 7
...
- **桌面系统托盘**：关闭窗口可最小化到托盘；托盘菜单（显示 / 新建 / 退出）；双击托盘图标显示窗口
- **全局快捷键**：`Ctrl+Shift+O` 显示窗口，`Ctrl+Shift+N` 新建文档

### 变更

- 设置项：关闭时最小化到托盘、启用全局快捷键
- CI 安装 Linux 托盘依赖（GTK、AppIndicator、libxdo）
- 版本号升至 **0.8.1**
...
- **脚注 / TOC 设置开关**：桌面与 Web 设置面板可关闭目录与脚注；导出 HTML/PDF 同步尊重设置
- **Web 最近文件存内容**：打开/下载时保存正文至 IndexedDB，点击最近项可重新打开（无需已开标签）
- **IndexedDB 大文档**：标签内容超过约 1.5MB 时自动迁移至 IndexedDB，避免 localStorage 配额溢出

### 变更

- `omd-common` 新增 `MarkdownRenderOptions`；版本号升至 **0.6.0**
