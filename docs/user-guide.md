...
omd 是一款用 Rust 编写的轻量级 Markdown 编辑器，提供**桌面版**和 **Web 版**两个版本，满足不同使用场景。

## 版本选择

| 特性 | 桌面版 | Web 版 |
|------|--------|--------|
| 运行环境 | 本地电脑（Windows/macOS/Linux） | 任意现代浏览器 |
| 安装 | 需编译或下载二进制 | 无需安装，打开即用 |
| 文件管理 | 原生文件对话框，直接读写磁盘 | 导入/下载 `.md` 文件 |
| 自动保存 | 手动保存（`Ctrl+S`） | 自动保存到浏览器 |
| Mermaid 图表 | ❌ 暂不支持 | ✅ 支持 |
| 图片粘贴 | ❌ | ✅ 支持粘贴/拖拽 |
| 手机使用 | ❌ | ✅ 响应式布局 |
| 离线使用 | ✅ | 部署后可离线 |
