...
与桌面版字段基本相同（Web 无自动保存到磁盘、无托盘项），通过界面 **⚙ 设置** 修改并自动保存。主要字段：
...
| `spell_check` | `true` | Web 编辑区浏览器拼写检查 |
| `custom_preview_css` | `""` | 注入预览与导出 HTML 的 CSS |
| `compress_images` | `true` | 粘贴/上传时压缩图片 |
| `keybinding_mode` | `"standard"` | `standard` / `vim` / `emacs` |

完整字段与桌面版共有项见 `web/src/settings.rs`、`src/settings.rs`。
