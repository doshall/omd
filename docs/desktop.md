...
| Export HTML… | — | 导出为独立 HTML 文件 |
| Exit | — | 退出应用（有未保存修改时会确认） |
...
- 支持多行编辑、滚动
- 左侧 **行号栏**：显示行号，光标所在行高亮
- 右侧 **Minimap** 缩略条：色块标示标题、代码块、图片等结构；点击或拖拽快速定位
- **查找 / 替换**：`Ctrl+F` 查找，`Ctrl+H` 替换；支持 `Enter` / `Shift+Enter` / `F3` 跳转匹配项
- **拖拽图片**：将图片文件拖入编辑区，在光标处插入 Markdown
- **自动保存**：View → Settings… 可开启，对已保存路径的文件定期写入磁盘
- **未保存确认**：关闭窗口、退出、新建或打开文件时，若有修改会提示保存
- **同步滚动**：分栏模式下编辑区与预览区按滚动比例联动
- **设置**：View → Settings… 可配置行号、Minimap、同步滚动、字号行高、专注模式等
- 自动获得焦点
...
- 代码块支持语法高亮（Rust、Python、JavaScript 等常见语言）
